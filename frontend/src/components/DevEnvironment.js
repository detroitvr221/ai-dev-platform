...
export function DevEnvironment({ selectedProjectId, onSelectProject, projectApi }) {
    ...
    return (_jsxs("div", { style: { display: 'grid', gridTemplateColumns: '260px 1fr 360px', height: '100%' }, children: [_jsxs("div", { style: { borderRight: '1px solid #eee', overflow: 'auto' }, children: [_jsxs("div", { style: { padding: 8, display: 'flex', gap: 8 }, children: [_jsx("select", { value: selectedProjectId ?? '', onChange: (e) => onSelectProject(e.target.value), style: { flex: 1 }, children: projectApi.projects.map((p) => (_jsx("option", { value: p.id, children: p.name }, p.id))) }), _jsx("button", { onClick: () => projectApi.createProject().then((p) => onSelectProject(p.id)), children: "New" })] }), _jsx(FileExplorer, { tree: tree, onSelect: (p) => setSelectedFilePath(p) })] }), _jsxs("div", { style: { display: 'grid', gridTemplateRows: '1fr 240px' }, children: [_jsx(CodeEditor, { filePath: selectedFilePath ?? '', value: fileContent, onChange: setFileContent, onSave: onSave }), _jsx(AgentStatus, {})] }), _jsxs("div", { style: { borderLeft: '1px solid #eee', display: 'grid', gridTemplateRows: '32px 1fr' }, children: [_jsxs("div", { style: { borderBottom: '1px solid #eee', display: 'flex', gap: 8, padding: '4px 6px' }, children: [_jsx(Tab, { label: "Chat", activeKey: mode, tabKey: "chat", onClick: () => setMode('chat') }), _jsx(Tab, { label: "Preview", activeKey: mode, tabKey: "preview", onClick: () => setMode('preview') })] }), _jsxs("div", { children: [mode === 'chat' && _jsx(ChatInterface, { selectedProjectId: selectedProjectId }), mode === 'preview' && selectedProjectId && (_jsx(PreviewPane, { projectId: selectedProjectId, projectApi: projectApi }))] })] })] }));
}
